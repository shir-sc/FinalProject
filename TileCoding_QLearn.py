...
#class QValueFunction:
    ...
    def getQValue (self, state, action):
        ...
        for coding, q_table in zip(state_codings, self.q_tables):
            # for each q table
            value += q_table[tuple(coding) + (state[1],) + (state[2],) + (state[3],) + (state[4],) +(action_idx,)]
    ...
    def updateQTable(self, state, action, reward, newState):
        ...
        for coding, q_table in zip(state_codings, self.q_tables):
            oldValue = q_table[tuple(coding) + (state[1],) + (state[2],) + (state[3],) + (state[4],) + (action_idx,)]
            delta = value - oldValue
            q_table[tuple(coding) + (state[1],) + (state[2],) + (state[3],) + (state[4],) + (action_idx,)] += self.alpha * delta

    def calcState(self, robot, ball, other_robot = None):
        ...
        if other_robot:
            HumanRobotPos =  int((3 * (other_robot.R_cell_y - 1)) / 9)
        else:
            HumanRobotPos = 1
